...
export class ResumeGeneratorComponent {
  ...
  extractAndStoreCertifications(): void {
    this.resume.certifications =
      this.resume.certifications + this.certfEditorHtml;
  ...
  extractAndStoreAchievements(): void {
    this.resume.achievements =
      this.resume.achievements + this.achivementEditorHtml;
  ...
  constructor(
    ...
    this.templateName = this.ar.snapshot.params['template'];
  }
  ...
  ngOnInit() {
    ...
    this.resumeBuilder.setTemplate(this.templateName);
    this.templateName = Number(this.ar.snapshot.params['template']);
  ...
  // Remove the local html2canvas stub, since we are using the actual library now.
}
